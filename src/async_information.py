...
class Information:
    ...
    async def __call__(self, unit_id: Union[str, int, None] = None) -> dict:
...
class RecurseInformation:
    ...
    async def _get_data(self) -> list[dict, ...]:
        ...
        for item in unit_all_data:
            tasks.append(self.fixed_endpoint(unit_id=item[id_prefix]))
        total_tasks: int = len(tasks)
        try:
            for task in track(asyncio.as_completed(tasks), description=f"Getting {self.fixed_endpoint.unit_name} data:",
                              total=total_tasks):
